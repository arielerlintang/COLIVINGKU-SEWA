...
class Sewa extends CI_Controller {
	...
	function tambahhari() {
		...
		if (!empty($sewa_rencana)) {
			...
			foreach ($dateRange as $date){
				...
				$this->db->set('pakai', $pakai);
				$this->db->set('id_rencana', $this->input->post("id_rencana"));
				$this->db->set('id_kamar', $this->input->post("id_kamar"));
				$this->db->insert('sewa_detail');
			}

			$this->session->set_flashdata('pesan_sukses', 'Tambahan tanggal sewa tersimpan');
	...
	function tambahrencana() { 
		...
		$rencana = [
			...
			'perpanjang_rencana' => '-',
			'jenis_rencana'      => 'B',
			'rencana_checkin'    => date('Y-m-d H:i:s', strtotime($this->input->post("mulai"))),
			'rencana_checkout'   => date('Y-m-d H:i:s', strtotime($this->input->post("hingga"))),
			'realisasi_checkin'  => date('Y-m-d H:i:s', strtotime($this->input->post("mulai"))),
			'realisasi_checkout' => date('Y-m-d H:i:s', strtotime($this->input->post("hingga")))
		];

		
		$this->db->insert('sewa_rencana', $rencana);
		...
		if (!empty($sewa)) {
			foreach ($dateRange as $date) {
				...
				$this->db->insert('sewa_detail', $detail);
			}
